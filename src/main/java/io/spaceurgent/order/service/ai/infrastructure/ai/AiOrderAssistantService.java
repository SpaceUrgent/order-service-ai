...
import io.spaceurgent.order.service.ai.infrastructure.ai.agent.ComplianceAgent;
import io.spaceurgent.order.service.ai.infrastructure.ai.agent.SqlAgent;
...
public class AiOrderAssistantService implements OrderAssistantService {
    ...
    public MessageDto ask(MessageDto userMessage) {
        ...
        final var resultContent = result.content();
        log.debug("Result: {}", resultContent);
        return new MessageDto(resultContent);
    }
}
